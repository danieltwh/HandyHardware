    ...
    def __init__(self, data, *args, **kwargs):
        ...
        for (requestId, name, model, issue, requestStatus) in data:
            requestId_label = tk.Label(self.frame, text=str(
                requestId), anchor="w", borderwidth=2, relief="groove", padx=10)
            name_label = tk.Label(self.frame, text=str(
                name), anchor="w", borderwidth=2, relief="groove", padx=10)
            model_label = tk.Label(self.frame, text=str(
                model), anchor="w", borderwidth=2, relief="groove", padx=10)
            issue_label = tk.Label(self.frame, text=str(
                issue), anchor="w", borderwidth=2, relief="groove", padx=10)
            requestStatus_label = tk.Label(self.frame, text=str(
                requestStatus), anchor="w", borderwidth=2, relief="groove", padx=10)
            ...
            if requestStatus == "Submitted":
                action_button = tk.Button(
                    self.frame, text="Approve", command=lambda requestId=requestId: self.delete(requestId))
...
class Admin_Request_Page_Header(tk.LabelFrame):
    def __init__(self, master, *args, **kwargs):
        ...
        tab1 = tk.Button(self, text="All Request",
                         command=lambda: master.refresh("All"))
        tab1.grid(row=0, column=0, padx=(10, 5))

        tab2 = tk.Button(self, text="Pending",
                         command=lambda: master.refresh("Pending"))
        # tab2.pack(side="left", fill="both")
        tab2.grid(row=0, column=1, padx=5)

        tab3 = tk.Button(self, text="My Jobs",
                         command=lambda: master.refresh("My Jobs"))
        # tab2.pack(side="left", fill="both")
...
class Admin_Request_Page(Frame):
    ...
    def show_header():
        header = Admin_Request_Page_Header(
            self, borderwidth=0, highlightthickness=0, pady=10)
